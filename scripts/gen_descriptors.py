...
The device is a USB Audio Class 1.0 playback sink: 384 kHz, 32-bit, stereo, on
an isochronous OUT endpoint, with an explicit feedback endpoint so the host can
follow the board's own audio clock (which is 383966.62 Hz, not 384000).

High speed only.  384 kHz stereo at four bytes a sample is 3.07 MB/s, and a
full-speed frame carries at most 1023 isochronous bytes -- 1.02 MB/s.  A
high-speed microframe carries 384 bytes of this comfortably.
"""
...
# One microframe of audio plus a sample of slack, since an asynchronous sink
# is asked for a varying number of samples each time.  High speed schedules
# eight microframes per millisecond.
AUDIO_MAX_PACKET = (SAMPLE_RATE // 8000 + 1) * CHANNELS * BYTES_PER_SAMPLE
...
device = desc(
    DEVICE,
    u16(0x0200),          # bcdUSB 2.00: required to be offered high speed
...
# A high-speed capable device must be able to describe what it would look like
# at the other speed.  This one only works at high speed -- the bandwidth is
# not there otherwise -- so the qualifier says one configuration and the host
# gets a stall if it asks for the other-speed configuration itself, which is
# how it learns there is nothing usable at full speed.
device_qualifier = desc(
    DEVICE_QUALIFIER,
    u16(0x0200),          # bcdUSB
    u8(0), u8(0), u8(0),  # class, subclass, protocol
    u8(64),               # bMaxPacketSize0
    u8(1),                # bNumConfigurations
    u8(0),                # bReserved
)
...
ep_audio = desc(
    ...
    u8(1),                # bInterval: every microframe
    u8(0),                # bRefresh
...
# bmAttributes 0x11 is isochronous with usage type "feedback".  At high speed
# the rate is four bytes of 16.16 samples per microframe rather than three of
# 10.14 per frame, and the period comes from bInterval as 2**(bInterval-1)
# microframes: 4 gives 8 microframes, one millisecond.  bRefresh is the
# full-speed way of saying the same thing and is not used here.
ep_feedback = desc(
    ENDPOINT,
    u8(EP_FEEDBACK_IN),
    u8(0x11),
    u16(4),               # wMaxPacketSize: a 16.16 rate in four bytes
    u8(4),                # bInterval: every 8 microframes
    u8(0),                # bRefresh
    u8(0),                # bSynchAddress
)
...
# ---------------------------------------------------------------- emit
blocks = [("DEVICE", device), ("CONFIG", config), ("QUALIFIER", device_qualifier)]
...
-- isochronous OUT endpoint of {AUDIO_MAX_PACKET} bytes per microframe, with an asynchronous
