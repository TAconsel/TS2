...
--
-- With Fs = 48000.5297 Hz and PHASE_BITS = 24 that is 349.52148 per Hz,
-- giving a tuning resolution of 2.9 mHz.

entity tone_gen is
    Generic (
        -- 1 kHz: round(1000 * 2**24 / 48000.5297) = 349521  ->  999.9986 Hz
        PHASE_INC    : natural := 349521;
    ...
    Port (
        ...
        -- 24-bit signed sample, MSB aligned for the I2S transmitter.
...
begin
    ...
    -- 16-bit table value in the top of the 24-bit word with the low 8 bits
    -- left at zero.  A shift by 0 is legal, so ATTEN_SHIFT = 0 needs no
    -- special case.
    attenuated <= shift_right(rom_data, ATTEN_SHIFT);
    sample     <= shift_left(resize(attenuated, 24), 8);
